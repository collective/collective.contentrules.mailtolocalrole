...
from collective.contentrules.mailtolocalrole import mailtolocalroleMessageFactory as _
...
class IMailLocalRoleAction(Interface):
    ...
    subject = schema.TextLine(title=_plone(u"Subject"),
                              ...
                              required=True)
    source = schema.TextLine(title=_plone(u"Email source"),
...
email. If no email is provided here, it will use the portal from address."),
                             required=False)
    localrole = schema.Choice(title=_(u"Local Role"),
                                description=_("Select a local role. \
The action will look up the all Plone site users who explicitly have this local \
...
class MailLocalRoleAction(SimpleItem):
    ...
    @property
    def summary(self):
        return _(u"Email report to users with local role ${localrole} on the object",
...
class MailActionExecutor(object):
    ...
    def __call__(self):
        localrole = self.element.localrole.strip()
...
action or enter an email in the portal properties'
        ...
        # search through all local roles on the object, and add users's email to the
        # recipients list if they have the local role stored in the action
...
class MailLocalRoleAddForm(AddForm):
    ...
    description = _plone(u"A mail action that can mail plone users who have a local role on the object")
